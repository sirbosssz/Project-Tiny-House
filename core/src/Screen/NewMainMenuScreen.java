...
public class NewMainMenuScreen extends AbstractScreen{
	...
	private Stage stage2, stage3, stage4, stage5;
	...
	public void render(float delta){
		...
		if(Setting.isPressed()&&keepState==0){
			keepState = 1;
			pressState = 1;
			 	//stage.addAction(new Actions().);
		}
		
		if(keepState == 1){
			stateTime += saveDelta;
			//System.out.println(keepState + " "+ stateTime);
			if(stateTime <= 0.1){
				Setting.moveBy(55*saveDelta*20, 20*saveDelta*30);
				posx += 1000*delta;//stage
				posx2 += 3000*delta;//background
				posx3 += 720*delta;
				camera.position.set(posx, posy, 0);
				//camera5.position.set(posx, posy, 0);
				camera2.position.set(posx2, posy2, 0);
				camera3.position.set(posx3, posy3,0);
				camera.update();
				camera2.update();
				camera3.update();
				//camera5.update();
			}
			else if(stateTime>0.1 && stateTime < 0.5){
				Setting.moveBy(25*saveDelta*20, 0);
				posx += 1000*delta;//stage
				posx2 += 3000*delta;//background
				posx3 += 720*delta;
				//posx4 += 700*delta;
		...
		if(keepState == 2){
			...
			if(stateTime < 1.12){
				stateTime += saveDelta;
				stage3.addAction(Actions.moveBy(0, 680*saveDelta, 0.2f));
				camera4.update();
				//System.out.println(keepState + " "+ stateTime);
			}
		...
		if(Setting.isPressed()&&keepState==3){
			keepState = 4;
		}
		if(keepState == 4){
			if(stateTime < 1.12){
		...
		if(keepState == 5){
			...
			//System.out.println(keepState + " "+ stateTime);
			if(stateTime < 0.39){
				Setting.moveBy(-26.0f*saveDelta*20,0f);
				Setting.moveBy(-0.08f*saveDelta*20,0f);
				posx -= 1000*delta;//stage
				posx2 -= 3000*delta;//background
				posx3 -= 720*delta;
				//posx4 -= 700*delta;
				//camera5.position.set(posx4, posy4, 0);
				camera.position.set(posx, posy, 0);
				camera2.position.set(posx2, posy2, 0);
				camera3.position.set(posx3, posy3,0);
				camera.update();
			...
			else if(stateTime < 0.48){
				Setting.moveBy(-55f*saveDelta*20, -20f*saveDelta*30);
				posx -= 1000*delta;//stage
				posx2 -= 3000*delta;//background
				posx3 -= 720*delta;
				//posx4 += 700*delta;
